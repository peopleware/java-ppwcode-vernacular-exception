package be.peopleware.struts_III.persistentBean.tag;
...
import be.peopleware.struts_III.persistentBean.CrudDynaActionForm;
 ...
 */
 public class EnumerationFieldTag extends CrudDynaActionFormTagSupport {
  ...
  private static final String DOUBLE_QUOTE_SPACE = "\" "; //$NON-NLS-1$
  ...
  public void doTag() throws IOException {
    ...
    if (getActionForm().isViewModeEdit()) {
      if (getPropertyEditor().getLabelsMap().isEmpty()) {
        ...
        // Field.properties | message.listField.noOptions
      }
      else {
  ...
  private void generateList(final String otherAttributes,
    ...
    out.println("<select " //$NON-NLS-1$
                + "name=\"" + getProperty() + DOUBLE_QUOTE_SPACE //$NON-NLS-1$ 
                + "id=\"" + getProperty() +  DOUBLE_QUOTE_SPACE //$NON-NLS-1$ 
                + "size=\"" + getSize() +  DOUBLE_QUOTE_SPACE //$NON-NLS-1$ 
                + otherAttributes
                + ">"); //$NON-NLS-1$
    while (iter.hasNext()) {
      ...
      if (getDontShowTags().indexOf(tag) == -1) {
        out.println("<option " //$NON-NLS-1$
                  + "value=\"" + tag +  DOUBLE_QUOTE_SPACE //$NON-NLS-1$ 
                  + writeSelected(tag, "selected") //$NON-NLS-1$
                  + ">" //$NON-NLS-1$
    ...
    while (iter.hasNext()) {
      ...
      if (getDontShowTags().indexOf(tag) == -1) {
        out.print("<input type=\"radio\" " //$NON-NLS-1$
                  ...
                  + "id=\"" + getProperty() +  DOUBLE_QUOTE_SPACE //$NON-NLS-1$ 
                  + otherAttributes
                  + writeSelected(tag, " checked") //$NON-NLS-1$
                  + " />" //$NON-NLS-1$
